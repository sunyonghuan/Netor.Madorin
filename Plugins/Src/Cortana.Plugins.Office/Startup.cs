...
[Plugin(
    ...
    Instructions = """
        使用 word_ 开头的工具操作 Word 文档，使用 excel_ 开头的工具操作 Excel 工作簿，使用 ppt_ 开头的工具操作 PowerPoint 演示文稿。
        操作 Word 前应先调用 word_get_outline 获取文档结构以确定目标段落索引。
        操作 Excel 前应先调用 excel_list_sheets 获取工作表列表以确定目标工作表。
        操作 PowerPoint 前应先调用 ppt_list_slides 获取幻灯片列表以确定目标幻灯片。
