...
> 仓库位置：`Plugins/Src/Madorin.Plugins.CSharpScript/`

---

## 一、定位与核心决策

让用户**无需安装 .NET SDK，仅安装 .NET Runtime** 即可执行 C# 脚本（`.csx`）。插件本身是一个普通的 .NET 10 exe，以子进程形式由 Madorin 主进程按 `runtime: process` 契约启动，通过 stdio JSON-RPC 通信。
...
```
Plugins/Src/Madorin.Plugins.CSharpScript/
├─ Madorin.Plugins.CSharpScript.csproj
...
```powershell
dotnet publish Plugins/Src/Madorin.Plugins.CSharpScript -c Release -r win-x64 --self-contained false
```
...
与框架对齐（见 `Src/Netor.Madorin.Plugin/Native/NativeHostProtocol.cs`）。**每行一个 JSON**，经由 stdin/stdout。
