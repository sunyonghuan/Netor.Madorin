...
为 Madorin Native 通道提供一个基于 **Attribute + Source Generator** 的开发框架，让开发者只需编写业务类和方法，框架自动生成所有导出函数、参数解析、路由分发和 `plugin.json` 清单文件。
...
Source Generator 编译时自动生成（70% 胶水代码）
        ↓
madorin_plugin_get_info / init / invoke / free / destroy
        ↓
plugin.json 清单文件
```
...
{
    [UnmanagedCallersOnly(EntryPoint = "madorin_plugin_get_info")]
    ...
    [UnmanagedCallersOnly(EntryPoint = "madorin_plugin_invoke")]
...
public interface IPluginBuilder
{
    ...
    /// 从宿主 madorin_plugin_init 传入的 config JSON 解析而来。
...
| 服务类型 | 说明 | 来源 |
|---------|------|------|
| `PluginSettings` | 插件配置（DataDirectory/WorkspaceDirectory/WsPort） | `madorin_plugin_init` 传入的 config JSON |
...
```csharp
// <auto-generated/>
// 由 Netor.Madorin.Plugin.Native.Generator 自动生成
...
public static partial class Startup
{
    ...
    [UnmanagedCallersOnly(EntryPoint = "madorin_plugin_free")]
    ...
    // ──────── madorin_plugin_destroy ────────
    // 停止 IHostedService → Dispose ServiceProvider

    [UnmanagedCallersOnly(EntryPoint = "madorin_plugin_destroy")]
...
```
Src/Plugins/
├── Netor.Madorin.Plugin.Native/                  # Attribute + 运行时服务
│   │                                             # 目标框架：net10.0
...
│
└── Netor.Madorin.Plugin.Native.Generator/        # 新增：Source Generator
    │                                             # 目标框架：netstandard2.0
...
<!-- 开发者的 csproj -->
<ItemGroup>
    <PackageReference Include="Netor.Madorin.Plugin.Native" Version="1.0.0" />
</ItemGroup>
```
...
```
Netor.Madorin.Plugin.Native.1.0.0.nupkg
├── lib/
│   └── net10.0/
│       └── Netor.Madorin.Plugin.Native.dll       # Attribute + PluginSettings + IPluginBuilder
│                                                  # 依赖：Microsoft.Extensions.DependencyInjection.Abstractions
│                                                  #        Microsoft.Extensions.Hosting.Abstractions
├── analyzers/
│   └── dotnet/
│       └── cs/
│           └── Netor.Madorin.Plugin.Native.Generator.dll  # Source Generator
└── build/
    └── Netor.Madorin.Plugin.Native.props          # 自动配置（PublishAot=true 等）
```
...
```xml
<!-- build/Netor.Madorin.Plugin.Native.props -->
<Project>
  <PropertyGroup>
    <!-- 确保 AOT 发布 -->
...
| 阶段 | 内容 | 产出 |
|------|------|------|
| **P0** | Attribute 定义 + PluginSettings + IPluginBuilder | `Netor.Madorin.Plugin.Native` 项目 |
