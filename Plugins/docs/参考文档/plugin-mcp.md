...
MCP（Model Context Protocol）通道允许 Madorin 作为 MCP 客户端连接到外部 MCP Server，将远程服务的工具能力无缝集成到 AI Agent 的工具链中。

与 Dotnet 和 Native 通道不同，MCP 通道不需要编写插件代码，只需在 UI 中配置 MCP Server 的连接信息即可。Madorin 通过 `ModelContextProtocol` SDK（1.2.0）自动发现并注册远程工具。
...
适用于本地命令行工具。Madorin 以子进程方式启动 MCP Server，通过 stdin/stdout 通信。
...
**实现细节：** Madorin 使用 `StdioClientTransport` 创建传输层：
...
连接成功后，Madorin 自动调用 `ListToolsAsync()` 获取远程工具列表。每个工具以 `McpClientTool`（实现 `AITool` 接口）的形式注册到 AI Agent 的工具链中。
