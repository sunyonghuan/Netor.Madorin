...
Native 通道支持使用 C/C++、Rust 或 C# AOT 编写原生 DLL 插件。插件 DLL 在独立的 `Madorin.NativeHost.exe` 子进程中通过 `NativeLibrary.Load` 加载，与宿主之间通过 stdin/stdout JSON 协议通信。
...
```
Madorin 宿主进程                          Madorin.NativeHost.exe 子进程
┌─────────────────┐     stdin/stdout     ┌──────────────────────────┐
...
| 函数 | 签名 | 说明 |
|------|------|------|
| `madorin_plugin_get_info` | `char* ()` | 返回插件信息 JSON（UTF-8），由宿主调用 `madorin_plugin_free` 释放 |
| `madorin_plugin_invoke` | `char* (char* toolName, char* argsJson)` | 调用工具，返回结果字符串，由宿主释放 |
| `madorin_plugin_free` | `void (char* ptr)` | 释放由 `get_info` / `invoke` 返回的字符串内存 |

### 可选导出

| 函数 | 签名 | 说明 |
|------|------|------|
| `madorin_plugin_init` | `int (char* configJson)` | 初始化插件，返回非 0 表示成功 |
| `madorin_plugin_destroy` | `void ()` | 清理资源（进程退出前调用） |
...
```
.madorin/plugins/my-native-plugin/
├── plugin.json
...
EXPORT const char* madorin_plugin_get_info(void) {
    // 返回堆分配的 JSON 字符串
...
EXPORT const char* madorin_plugin_invoke(const char* toolName, const char* argsJson) {
    char* result = (char*)malloc(256);
    snprintf(result, 256, "工具 %s 调用成功", toolName);
    return result;
}

EXPORT void madorin_plugin_free(char* ptr) {
    free(ptr);
}
```
...
1. **内存管理** — `get_info` 和 `invoke` 返回的字符串必须通过 `madorin_plugin_free` 释放，确保分配器匹配
