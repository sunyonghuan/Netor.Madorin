...
4. install-plugin-package.ps1（内部会转发到仓库内 `skills/skill-plugin-installation/scripts/install-package.ps1`）。
...
- 安装能力依赖仓库内 `skills/skill-plugin-installation` 技能；调用路径必须使用相对脚本，不要写死机器绝对路径。
