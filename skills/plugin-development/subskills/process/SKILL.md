...
2. 在 `Startup.cs` 中声明 `[Plugin]` 入口；如果需要宿主权限申请或宿主设置界面，再叠加 `[RequiredHostCapability]`、`[PluginSetting]`，然后注册依赖，在 `Tools/` 中编写 `[Tool]` 类和方法。
...
5. 先执行 `dotnet build`，确认 `obj/.../generated/.../Program.g.cs`、`{PluginClass}Debugger.g.cs` 和 `bin/.../plugin.json` 都已生成；如果声明了宿主能力或设置项，同时检查 `requiredHostCapabilities` / `settingsSchema` 已正确输出。
...
- 入口类可以叠加多个 `[RequiredHostCapability]` 和 `[PluginSetting]`，它们会分别生成 `requiredHostCapabilities` 与 `settingsSchema`。
...
- Process 通道中 `PluginSettings` 会在 `init` 后注入；依赖它的工具、服务和日志只能在 `InitAsync()` 之后使用。
- 宿主能力声明仅用于授权摘要和设置界面，不代表宿主一定已授予；工具逻辑必须处理未授予场景。
- 设置界面字段必须使用 `[PluginSetting]` 声明，不要另造自定义设置清单；敏感项不得输出到 stdout/stderr/文件日志。
...
上面的声明会进入生成的 `plugin.json` / `get_info` 元数据中，而不是要求业务代码自己返回协议对象。
