...
9. **技能作用域选择** - 创建技能前必须确认创建全局技能还是项目技能
10. **技能目录获取** - 创建技能前必须调用目录工具获取全局技能目录和工作目录技能目录
---
## Out of Scope
以下功能**不在**本技能范围内：
...
### 创建位置选择
当用户要求创建技能时，必须先询问用户要创建哪一种技能：

1. **全局技能** - 适用于所有工作区，必须放在全局技能目录下。
2. **项目技能** - 仅适用于当前工作区，必须放在工作目录技能目录下。

创建技能前必须调用以下工具获取实际目录，不要手写或猜测路径：

| 技能类型 | 必须调用的工具 | 目录用途 |
|----------|----------------|----------|
| 项目技能 | `sys_get_workspace_skills_directory` | 获取当前工作目录技能目录 |
| 全局技能 | `sys_get_user_skills_directory` | 获取当前用户全局技能目录 |

推荐流程：

1. 询问用户：`要创建全局技能还是项目技能？`
2. 调用 `sys_get_workspace_skills_directory` 获取项目技能目录。
3. 调用 `sys_get_user_skills_directory` 获取全局技能目录。
4. 根据用户选择，在对应目录下创建 `{skill-name}\SKILL.md`。
5. 创建完成后回显实际写入路径，便于用户验证。
...
- [ ] 创建技能前已询问用户选择全局技能或项目技能
- [ ] 已调用 `sys_get_workspace_skills_directory` 获取工作目录技能目录
- [ ] 已调用 `sys_get_user_skills_directory` 获取全局技能目录
- [ ] 已根据用户选择写入正确目录
---
...
1. 询问用户：`要创建全局技能还是项目技能？`
2. 调用 `sys_get_workspace_skills_directory` 获取工作目录技能目录。
3. 调用 `sys_get_user_skills_directory` 获取全局技能目录。
4. 根据用户选择确定目标目录：
    - 全局技能：`{全局技能目录}\wp-plugin-dev`
    - 项目技能：`{工作目录技能目录}\wp-plugin-dev`
5. 在目标目录创建 `SKILL.md`
6. 复制标准模板到 `SKILL.md`
7. 修改 `name` 为 `wp-plugin-dev`
8. 修改 `description` 为 `WordPress plugin development skill`
9. 填写业务内容（Overview, Routing Table, Scope 等）
10. 回显实际写入路径，确认创建完成
### 示例 2: 检查技能格式
**用户**: "检查一下我的技能格式对不对"
...
7. **备份原文件** - 修改前先备份原始内容
8. **先确认作用域** - 创建技能前必须确认全局技能或项目技能
9. **使用工具取目录** - 必须通过 `sys_get_workspace_skills_directory` 和 `sys_get_user_skills_directory` 获取目录
---
*Version: 1.0*
