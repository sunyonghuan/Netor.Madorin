...
| `subskills/transfer-rules.md` | 文件传输操作 | 上传/下载文件、同步目录、SFTP 操作时 |

## SSH connection Rules
- Use the dedicated SSH plugin tools only.
- For reusable sessions, call `ssh_connect` with host, port, user, authMethod, and privateKeyPath/passphrase as needed.
- Execute commands with `ssh_session_exec`; use `ssh_exec_script` for multi-line scripts.
- For one-off checks, `ssh_run_once` or `ssh_run_script_once` is acceptable.
- Always close reusable sessions with `ssh_disconnect` after execution.
- Set timeoutMs to 30000 for standard commands and increase it for slow operations.
- Sanitize all command inputs to prevent injection.
- Do not invoke local PowerShell tools or local `ssh`/`ssh.exe`/`scp` processes for remote server operations.
