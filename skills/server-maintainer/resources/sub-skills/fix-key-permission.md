...
本子技能只处理本地 Windows 私钥文件 ACL。远程连接验证必须使用 SSH 插件 `ssh_connect`，不要通过 PowerShell 启动 `ssh`。
...
# 步骤 2：执行本地权限修复脚本（替换脚本中的 {ip地址} 为实际 IP）
powershell.exe -ExecutionPolicy Bypass -File "E:\Workspace\.cortana\skills\server-maintainer\scripts\fix-key-permission.ps1"

# 步骤 3：使用 SSH 插件 ssh_connect 验证连接
```
...
6. 验证权限设置是否成功
7. 可选：使用 SSH 插件 `ssh_connect` 测试 SSH 连接
8. 返回执行结果
...
- `fix-key-permission.ps1`: 仅用于本地 Windows 私钥 ACL 修复，不用于远程服务器管理。
