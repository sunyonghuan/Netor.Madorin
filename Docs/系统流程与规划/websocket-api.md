...
```json
{
  ...
  "version": "1.4.0",
  "topics": ["conversation", "memory", "model", "plugin", "workflow", "meeting"]
}
```

`clientId` 是当前 WebSocket 连接标识。当前 connected 消息列出 `conversation`、`memory`、`model`、`plugin`、`workflow`、`meeting`，客户端可按需订阅。
...
| `meeting.v1` | 会议模式事件与历史回放 |

未知 capability 会被忽略。订阅 `workflow` 但未声明 `workflow.v1` 时，当前实现只记录 warning，不阻断订阅。`meeting` topic 当前也采用兼容订阅策略。
...
```json
{
  ...
  "version": "1.4.0",
  "topic": "conversation",
  "op": "conversation.chat.generation.stop",
  "payload": {
    "type": "stop"
  }
}
...
```json
{
  ...
  "version": "1.4.0",
  "topic": "conversation",
  "op": "conversation.chat.token",
  "source": "host",
  "target": "3f1f2c7f3e2b4d4c9e1a7b8c9d0e1f2a",
  "timestamp": 1770000000000,
  "eventType": "conversation.chat.token",
  "payload": {
    "type": "token",
    "data": "当前"
  }
...
| op | payload.type | 说明 |
| --- | --- | --- |
| `conversation.chat.token` | `token` | 流式文本片段 |
| `conversation.chat.done` | `done` | 本轮回复完成，`payload.sessionId` 可能为空 |
| `conversation.chat.error` | `error` | 生成失败或取消 |
| `conversation.chat.stt_partial` 等 | 对应旧事件 type | 语音、TTS、唤醒等广播事件 |
...
会议模式通过 `meeting` topic 接入：

| op | 说明 |
| --- | --- |
| `meeting.event.publish` | 宿主发布会议创建、完成、取消和总结消息事件 |
| `meeting.history.replay` | 请求会议历史回放 |
| `meeting.history.batch` | 历史批次响应 |
| `meeting.history.completed` | 历史回放完成 |
...
```json
{
  ...
  "version": "1.4.0",
  "topic": "meeting",
  "op": "meeting.history.replay",
  "requestId": "meeting-replay-001",
  "payload": {
    "sinceTimestamp": 0,
    "batchSize": 500
  }
...
`meeting.history.batch` 返回已完成会议和会议总结消息记录；`meeting.history.completed` 返回本次回放总数。
...
| 裸 `token` / `done` / `error` 输出 | PluginBus `type=event` + `topic=conversation` + `op=conversation.chat.*` |
