# 执行计划(Kestrel WebSocket 服务重构) : 100%

> 当前代码状态：本计划的“多服务 Kestrel 替换”已被后续单端点 PluginBus 收口方案覆盖。当前运行实现是 `WebSocketPluginBusServerService`，通过 Kestrel 监听 `CortanaWsEndpoints.PluginBusPath = "/internal"`，统一承载聊天输入输出、conversation 历史回放、memory supply、model capability 和 workflow topic。旧 `/ws/`、`/internal/conversation-feed/`、独立 Feed 服务与独立 Interaction 服务不再是当前代码结构。

## 背景

历史背景中提到的独立聊天服务与独立 feed 服务已经不再作为当前架构存在。当前代码保留的是统一 `WebSocketPluginBusServerService`，并使用 `KestrelWebSocketHost`、`WebSocketConnectionManager`、`WebSocketHeartbeatLoop` 等基础设施处理生命周期、连接管理、心跳与发送。

本文后续内容保留为历史实施记录；判断当前功能与进度时，以本文顶部“当前代码状态”和 `Docs/系统流程与规划/websocket-api.md` 为准。
...
长期记忆供应与 model-capability 属于交互控制面，应归入 `WebSocketPluginBusServerService` 所代表的交互功能板块；Feed 服务只保留会话事实广播能力。
...
### WebSocketPluginBusServerService 新设计
...
路径：
- `CortanaWsEndpoints.ChatPath`，例如 `/internal`
- 兼容保留现有 `ModelCapabilityProtocol.Path`。
- 兼容保留现有长期记忆供应消息通道；若当前依赖 PluginBus 承载，应先在交互服务中提供兼容桥接，不直接删除旧入口。

输入协议：
- `send`：用户消息，进入 `WebSocketInputChannel` / `IAiChatEngine`。
- `stop`：停止当前生成。
- `system.notice`：系统通知。
- `memory.context.supply.response` / `memory.context.supply.error`：长期记忆供应响应。
...
路径：
- `CortanaWsEndpoints.PluginBusPath`，例如 `/internal`
...
- relay 调用 `IPluginBusBroadcaster.BroadcastAsync(message)`。
...
| 功能 | 最终归属 | 兼容要求 |
|---|---|---|
| Chat send/stop/token/done/error | `WebSocketPluginBusServerService` | 保持 `/internal` 与现有消息类型 |
| `model-capability` 控制面 | `WebSocketPluginBusServerService` 交互板块 | 保持现有 `ModelCapabilityProtocol.Path` 可用 |
| 长期记忆供应 | `WebSocketPluginBusServerService` 交互板块 | 保持现有 memory supply request/response/error 消息兼容 |
| 会话事实广播 | `WebSocketPluginBusServerService` | 保持 PluginBus 旧连接入口 |
...
- 新交互控制面在 `WebSocketPluginBusServerService` 下提供正式归属入口。
...
- `services.AddSingleton<IPluginBusBroadcaster, WebSocketPluginBusServerService>()`
...
当前：
- `WebSocketPluginBusServerService` 注册为 `IChatTransport`。
- `WebSocketPluginBusServerService` 注册为 `ILongMemorySupplyClient`。

目标：
- `WebSocketPluginBusServerService` 注册为 `IChatTransport`。
- `WebSocketPluginBusServerService` 或其交互控制面组件注册为 `ILongMemorySupplyClient`。
- `WebSocketPluginBusServerService` 注册为 `IPluginBusBroadcaster`。
- 迁移期内 `WebSocketPluginBusServerService` 可保留 legacy bridge，但不作为长期归属。

### Relay 调整

`WebSocketConversationFeedRelayService` 依赖从具体 `WebSocketPluginBusServerService` 改为 `IPluginBusBroadcaster`。
...
## Step 1 明确协议边界 : 0%
- [×] 确认 Chat 服务只保留 `/internal`。
- [×] 确认 Feed 服务只保留 `/internal`。
- [×] 确认长期记忆供应归属交互控制面。
...
- `WebSocketPluginBusServerService` 已接入 `KestrelWebSocketHost` 公共 Host 构建与释放流程。
- `WebSocketPluginBusServerService` 的 Chat 主通道已接入 `WebSocketConnectionManager`、`WebSocketConnection`、`WebSocketSendQueue` 与 `WebSocketHeartbeatLoop`。
- `WebSocketPluginBusServerService` 的 PluginBus legacy 通道已接入 `WebSocketConnectionManager`、`WebSocketConnection`、`WebSocketSendQueue` 与 `WebSocketHeartbeatLoop`。
- `WebSocketPluginBusServerService` 的 model-capability legacy 通道已接入 `WebSocketConnectionManager`、`WebSocketConnection`、`WebSocketSendQueue` 与 `WebSocketHeartbeatLoop`。
- `WebSocketPluginBusServerService` 已清理旧内联心跳循环和旧连接移除包装方法，统一使用公共连接基础设施。
- `WebSocketPluginBusServerService` 已接入 `KestrelWebSocketHost` 公共 Host 构建与释放流程。
- `WebSocketPluginBusServerService` 的 PluginBus 通道已接入 `WebSocketConnectionManager`、`WebSocketConnection`、`WebSocketSendQueue` 与 `WebSocketHeartbeatLoop`。
- `WebSocketPluginBusServerService` 的 model-capability legacy 通道已接入 `WebSocketConnectionManager`、`WebSocketConnection`、`WebSocketSendQueue` 与 `WebSocketHeartbeatLoop`。
- `WebSocketPluginBusServerService` 已清理旧内联心跳循环和 model-capability 旧发送锁残留。
- 已通过 `dotnet build Src/Netor.Cortana.Networks/Netor.Cortana.Networks.csproj` 构建验证。

## Step 3 重构 WebSocketPluginBusServerService : 89%
...
## Step 4 重构 WebSocketPluginBusServerService : 80%
- [×] 移除 HttpListener 和裸 Thread。
- [×] 只挂载 PluginBusPath。
- [ ] 移除客户端业务消息处理。
...
- [×] 实现 IPluginBusBroadcaster。
- [×] 接入 KestrelWebSocketHost 公共 Host 构建与释放流程。
- [×] PluginBus 通道接入 WebSocketConnectionManager。
- [×] PluginBus 输出消息改为 WebSocketSendQueue 入队发送。
- [×] PluginBus 心跳改为 WebSocketHeartbeatLoop 驱动。
- [×] model-capability legacy 通道接入 WebSocketConnectionManager。
- [×] model-capability legacy 输出消息改为 WebSocketSendQueue 入队发送。
...
- [×] Feed PluginBus 通道接入连接管理器和发送队列后，临时探针回归通过。
- [×] Feed PluginBus 通道接入连接管理器和发送队列后，Networks 项目二次构建通过。
- [×] Chat 主通道接入连接管理器和发送队列后，临时探针回归通过。
- [×] Chat 主通道接入连接管理器和发送队列后，Networks 项目构建通过。
- [×] Interaction PluginBus legacy 通道接入连接管理器和发送队列后，临时探针回归通过。
- [×] Interaction PluginBus legacy 通道接入连接管理器和发送队列后，Networks 项目构建通过。
...
- 占用配置端口后，`WebSocketPluginBusServerService` 与 `WebSocketPluginBusServerService` 均未导致进程崩溃。
- 两个服务均回退到可用端口启动，且 `LastError` 在回退成功后保持为空。
- Chat 路径 `/internal` 可建立 WebSocket 连接并收到 `connected` 首帧。
- Feed 路径 `/internal` 可建立 WebSocket 连接并收到 `PluginBus` 的 `connected` 首帧。
...
- Feed PluginBus 通道改用 `WebSocketConnectionManager` / `WebSocketSendQueue` 后，握手、异常断开、新连接、双客户端广播、重启后握手均通过探针回归。
- Chat 主通道改用 `WebSocketConnectionManager` / `WebSocketSendQueue` 后，握手、异常断开、新连接、token/done/error 投递、双客户端广播、重启后握手均通过探针回归。
- Interaction PluginBus legacy 通道改用 `WebSocketConnectionManager` / `WebSocketSendQueue` 后，现有 Chat/Feed 握手、异常断开、双客户端广播、重启后握手均通过探针回归。
...
- Feed PluginBus 通道发送路径已改为连接管理器入队发送，服务停止、接收循环结束和心跳超时统一走连接管理器关闭流程。
- Chat 主通道发送路径已改为连接管理器入队发送，服务停止、接收循环结束和心跳超时统一走连接管理器关闭流程。
- Interaction PluginBus legacy 通道发送路径已改为连接管理器入队发送，服务停止、接收循环结束和心跳超时统一走连接管理器关闭流程。
- Interaction model-capability legacy 通道发送路径已改为连接管理器入队发送，服务停止、接收循环结束和心跳超时统一走连接管理器关闭流程。
- `WebSocketPluginBusServerService` 旧内联心跳循环、旧 `HasOpenClient` / `SendHeartbeatAsync` / `IsHeartbeatTimedOut` / `CloseHeartbeatTimedOutClient` 与旧 Remove 包装方法已删除。
- `WebSocketPluginBusServerService` 旧内联心跳循环、model-capability 旧字典发送锁与旧关闭逻辑已删除；model-capability legacy 通道改为连接管理器入队发送。
