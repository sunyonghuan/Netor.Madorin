...
flowchart TB
    ...
    Client[Cortana 客户端] --> Api[Minimal API]
...
```text
Netor.Cortana.Platform.slnx
```

项目结构建议：

```text
Src/Netor.Cortana.Platform/
...
Tests/Netor.Cortana.Platform/
  Netor.Cortana.Platform.Tests/
```
...
| 项目 | 类型 | 职责 |
|---|---|---|
| Netor.Cortana.Platform.Api | ASP.NET Core Minimal API | 对外 API，供 Web、Admin、Cortana 客户端调用 |
| Netor.Cortana.Platform.Web | ASP.NET Core MVC | 个人用户前台、市场、详情、登录注册、个人中心 |
| Netor.Cortana.Platform.Admin | ASP.NET Core MVC + Layui | 管理中心，资产维护、订单查看、订阅查看、用户查看 |
| Netor.Cortana.Platform.Services | Class Library | 业务服务、下载校验、订阅创建、订单处理、文件处理 |
| Netor.Cortana.Platform.Entitys | Class Library | 实体、枚举、EF Core DbContext、数据库迁移、实体配置 |
| Netor.Cortana.Platform.Core | Class Library | 通用基类、接口约束、分页、统一结果、异常、常量、扩展方法 |
| Netor.Cortana.Platform.Tests | Test | 关键服务和 API 的测试 |

## 6. 各项目内部结构

### 6.1 Netor.Cortana.Platform.Core
...
### 6.2 Netor.Cortana.Platform.Entitys
...
### 6.3 Netor.Cortana.Platform.Services
...
### 6.4 Netor.Cortana.Platform.Api
...
### 6.5 Netor.Cortana.Platform.Web
...
### 6.6 Netor.Cortana.Platform.Admin
...
- `Netor.Cortana.Platform.Entitys/DependencyInjection.cs`
- `Netor.Cortana.Platform.Services/DependencyInjection.cs`
...
```json
{
  "Database": {
    "Provider": "SqlServer",
    "ConnectionString": "Server=.;Database=CortanaPlatform;Trusted_Connection=True;TrustServerCertificate=True"
  }
}
```
...
| 目的 | 推荐名称 |
|---|---|
| 实体库 | `Netor.Cortana.Platform.Entitys` |
| 基础库 | `Netor.Cortana.Platform.Core` |
| 业务服务 | `Netor.Cortana.Platform.Services` |
| API | `Netor.Cortana.Platform.Api` |
| 前台 Web | `Netor.Cortana.Platform.Web` |
| 管理后台 | `Netor.Cortana.Platform.Admin` |
