...
v1.3.1 是 Cortana 向“模型入口自由化”迈出的一大步：新增 **Ollama 本地协议代理**，可以把 Cortana 中已经配置好的国产大模型、企业模型、私有 API 模型，通过 `http://localhost:11434` 伪装成本机 Ollama 模型暴露给外部工具。
...
一句话：**Cortana 不替代编辑器，Cortana 接管模型入口。**
...
Cortana 内置的 Ollama 协议代理可以把远程 API 包装为本地 Ollama 服务：

```text
http://localhost:11434
```

外部工具以为自己在调用本地模型，实际上请求会由 Cortana 转发到你配置好的国产模型、企业网关、私有 API 或任何 OpenAI-compatible 服务。

## 这不是造 IDE，这是释放现有编辑器生态

很多项目会选择重新做一个 AI IDE，但 Cortana 反过来：不重复造轮子，不和 VSCode / Visual Studio 抢饭碗。
...
Cortana 只负责一件更关键的事：**把用户真正想用的模型送进编辑器。**
...
它让 Cortana 可以作为“本地模型网关”存在，对外表现为 Ollama，对内接入任意已配置 AI 厂商。

### 2. 国产模型 / 企业模型 / 私有 API 统一暴露

只要模型已经在 Cortana 中配置，就可以通过 Ollama 协议对外暴露。
...
Cortana 在这里不做过度“翻译”，尽量少干预，让工具链保持原生兼容。

### 5. 模型名前缀隔离

对外暴露模型时统一使用 `cortana-` 前缀，避免和本机已有 Ollama 模型冲突。

例如：

```text
qwen-plus       -> cortana-qwen-plus
deepseek-chat   -> cortana-deepseek-chat
```

外部工具看到的是本地模型名，Cortana 内部会自动还原到真实模型名调用上游 API。
...
这让第三方工具更容易把 Cortana 暴露的云端模型识别为“本地可用模型”。
...
Cortana 把模型调度入口开放出来，成本和能力都由用户自己控制。
...
5. 拉取模型列表，确认可以看到 `cortana-` 前缀模型。
6. 发起聊天、代码生成或工具调用，确认请求经 Cortana 转发到目标模型。

## 一句话总结

v1.3.1 的意义不是“多了一个代理端口”，而是让 Cortana 成为模型自由入口：
