...
3. `conversation-feed`、模型能力控制面和聊天传输逐步统一到 `cortana.plugin-bus` 协议。
...
- 旧 `/ws/` 客户端如仍存在，需要按 PluginBus 新端点补充迁移说明。
...
- 旧 release 文档中提到的 `/internal/model-capability/` 和 `/internal/conversation-feed/` 后续可补充废弃说明。
