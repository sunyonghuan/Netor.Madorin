...
> 状态：正式发布文档。  
> 日期：2026-05-13  
> 用途：记录 v1.3.7 正式发布内容、主要变更和发布边界。

## 发布概览

v1.3.7 重点围绕 **单端点统一插件总线、Memory 插件总线迁移、长期记忆供应链路观测、对话流内实时过程卡片、系统提醒展示优化、主窗口状态持久化和版本整理** 展开。

本版本主要包含以下能力：

1. 主程序版本号从 `1.3.6` 提升到 `1.3.7`。
2. 内部 WebSocket 通道收敛为统一 PluginBus：`ws://localhost:{port}/internal`。
3. `PluginBus`、模型能力控制面和聊天传输逐步统一到 `cortana.plugin-bus` 协议。
4. Memory 插件从独立 PluginBus / model-capability 通道迁移为共享 PluginBus 长连接。
...
一句话：**本版本把内部插件通信从多端点、多协议收敛为一个插件总线，同时增强长期记忆链路可观测性，并补齐对话流过程展示和桌面窗口体验细节。**
...
相关端点与协议常量已收敛到 `CortanaWsEndpoints`，包括：
...
- `MemoryIngestService` 从“连接 PluginBus”调整为“连接 PluginBus”；
...
- 旧的 PluginBus 属性暂时保留为兼容调用点，但实际映射到 PluginBus。
...
本节用于明确 v1.3.7 的正式发布价值与发布边界。

- 明确统一 PluginBus 为内部插件通信主通道，旧多端点控制面逐步作为兼容路径处理。
- 明确 Memory 插件长期记忆供应、历史回放和模型能力调用均通过 PluginBus 长连接收敛。
- 明确对话流实时过程卡片用于运行过程展示，不改变聊天消息入库语义。
- 明确 `system.notice` 为主界面临时系统提示，不写入聊天历史、不触发 AI 对话，仅用于当前界面展示。
- 明确主窗口位置与大小持久化只保存普通窗口状态，避免最大化、最小化覆盖正常尺寸。
...
```text
dotnet build .\Src\Netor.Cortana.UI\Netor.Cortana.UI.csproj
```
...
- 旧 `/internal` 客户端如仍存在，需要按 PluginBus 新端点补充迁移说明。
- `PluginBus.Port` 与既有 `WebSocket.Port` 设置项之间的关系后续可在 UI 层进一步明确展示。
- Memory 插件的 PluginBus 长连接断线重连体验后续可继续增强。
- `conversationFeed*` 兼容属性后续版本可继续清理。
- 旧 release 文档中提到的 `/internal/model-capability/` 和 `/internal` 后续可补充废弃说明。

> 本发布说明作为 v1.3.7 正式发布文档，后续补丁或增量能力请在新版本发布说明中继续记录。
