...
7. 优化插件子进程退出治理，Windows 下使用 Job Object 跟踪插件子进程，降低 `Cortana.NativeHost.exe` 残留风险。
...
本版本没有新增独立 HTTP/WS 端点，而是复用 `/internal` 长连接承载轻量请求/响应：

```text
宿主 LongMemoryContextProvider
   → ILongMemorySupplyClient
   → WebSocketServerService / WebSocketPluginBusServerService
   → /internal memory.context.supply.request
   → MemoryIngestService
...
Native 插件当前由 `Cortana.NativeHost.exe` 子进程加载，每个 Native 插件对应一个宿主子进程。为避免主程序异常退出或托盘退出时留下大量插件进程，本版本补强子进程清理：
...
- UI 退出流程统一为“取消业务任务 → 卸载插件/MCP → 停止后台服务 → 释放 DI 容器”，降低 `Cortana.NativeHost.exe` 驻留风险。
...
- 插件子进程 Job Object 跟踪在 `Netor.Cortana.Plugin` 项目构建中通过。
