...
## Step 1 新增事件模型 : 100%
- [√] 在 `Src/Netor.Madorin.Entitys/Events.cs` 新增 `Events.OnSystemNotice`。
...
- [√] 在 `Src/Netor.Madorin.Networks/WebSockets/Servers/WebSocketServerService.cs` 保留现有 `type` / `data` 解析逻辑。
...
## Step 3 发布系统提示事件 : 100%
- [√] 在 `Src/Netor.Madorin.Networks/WebSockets/Channels/WebSocketInputChannel.cs` 增加 `case "system.notice"`。
- [√] 收到 `system.notice` 后发布 `Events.OnSystemNotice`。
- [√] 确保该分支不调用 `chatEngine.SendMessageAsync(...)`。
- [√] 当 `title`、`level`、`source` 缺省时使用默认值。

## Step 4 主界面临时显示 : 100%
- [√] 在 `Src/Netor.Madorin.UI/Views/MainWindow.axaml.cs` 订阅 `Events.OnSystemNotice`。
- [√] 在收到系统提示时隐藏欢迎面板。
- [√] 调用新增 UI 方法添加临时系统提示卡片。

## Step 5 系统提示卡片与折叠 : 100%
- [√] 在 `Src/Netor.Madorin.UI/Views/Main/MainWindow.Messaging.cs` 新增 `AddSystemNotice(...)`。
