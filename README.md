...
聊天客户端可以继续发送旧格式 `{"type":"send","data":"..."}` / `{"type":"stop"}`；插件和新客户端推荐使用 PluginBus envelope，通过 `chat.message.send`、`chat.generation.stop`、`conversation.history.replay` 等 operation 接入。服务端输出统一为 `type=event`、`topic=conversation`、`op=chat.token/chat.done/chat.error` 的事件消息。
