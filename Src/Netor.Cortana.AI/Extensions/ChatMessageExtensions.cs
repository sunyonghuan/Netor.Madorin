...
public static class ChatMessageExtensions
{
    ...
    private static JsonElement NormalizeToJsonElement(object? value)
    {
        ...
        string json = value switch
        {
            ...
            IEnumerable enumerable when value is not string => SerializeEnumerableArgument(enumerable),
    ...
    /// <summary>
    /// 手动序列化工具参数中的数组/集合，避免 AOT 运行时按具体数组类型查找缺失的 JSON 元数据。
    /// </summary>
    private static string SerializeEnumerableArgument(IEnumerable values)
    {
        var elements = new List<JsonElement>();
        foreach (var item in values)
        {
            elements.Add(NormalizeToJsonElement(item));
        }

        return JsonSerializer.Serialize(elements, PersistedArgumentsJsonContext.Default.ListJsonElement);
    }
...
[System.Text.Json.Serialization.JsonSerializable(typeof(System.Collections.Generic.List<System.Text.Json.JsonElement>))]
