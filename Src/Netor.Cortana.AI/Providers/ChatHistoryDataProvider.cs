...

{
    ...
    protected override async ValueTask StoreChatHistoryAsync(InvokedContext context, CancellationToken cancellationToken = default)
    {
        ...
        // 从 StateBag 取预生成的助手消息 ID，用于与 ChatMessageAssets 关联（图片回看）
        // AiChatHostedService 写入图片资源时使用同一个 ID，确保两侧一致。
        var prebuiltAssistantMsgId = context.Session?.StateBag.GetValue<string>("assistantmessageid");

        // 找到"最终 assistant 文本消息"的索引：ResponseMessages 中最后一条 Role=Assistant
        // 且不含 tool_calls 的消息。该消息对应用户看到的 AI 回复正文。
        int finalAssistantIndex = -1;
        for (int i = responseMessages.Count - 1; i >= 0; i--)
        {
            var m = responseMessages[i];
            if (m.Role != ChatRole.Assistant) continue;
            bool hasToolCalls = m.Contents.Any(c => c is FunctionCallContent or ToolCallContent or McpServerToolCallContent);
            if (!hasToolCalls) { finalAssistantIndex = i; break; }
        }
        ...
        var messages = responseMessages
            .Select((t, index) => new ChatMessageEntity
            {
                ...
                // 最终 assistant 文本消息优先使用预生成 ID（与 ChatMessageAssets 关联），其余消息生成新 Guid。
                // 不再把 assistant 消息强行统一为同一个 assistantMessageId，避免链条断裂。
                Id = (index == finalAssistantIndex && !string.IsNullOrEmpty(prebuiltAssistantMsgId))
                    ? prebuiltAssistantMsgId
    ...
    private async Task<string> AddOrUpdateSessionAsync(AgentSession? session, string title, AIAgent agent, bool accumulateInputTokens = false)
    {
        ...
        try
        {
            ...
            var agentName = session?.StateBag.GetValue<string>("agentid");
            if (!string.IsNullOrWhiteSpace(agentName))
            {
                sessionEntity.AgentName = agentName;
            }

            var sourceTaskId = session?.StateBag.GetValue<string>("worktaskid");
            if (!string.IsNullOrWhiteSpace(sourceTaskId))
            {
                sessionEntity.SourceTaskId = sourceTaskId;
            }
            ...
            dbContext.Execute(
                ...
                cmd =>
                {
                    ...
                    cmd.Parameters.AddWithValue("@AgentName", sessionEntity.AgentName);
                    cmd.Parameters.AddWithValue("@SourceTaskId", sessionEntity.SourceTaskId);
