...
internal sealed class ResourceCardPanel : WrapPanel
{
    ...
    private Control BuildCard(ChatMessageAssetEntity asset)
    {
        ...
        card.ContextMenu = ResourceContextMenuFactory.Create(
            card,
            fullPath,
            asset.OriginalName,
            isImage: false,
    ...
    private void OpenResource(ChatMessageAssetEntity asset)
    {
        ...
        {
            ShowStatus($"文件不存在：{asset.OriginalName}");
            return;
        }
        ...
        {
            ShowStatus($"无法打开：{ex.Message}");
        }
    }

    private void ShowStatus(string? message)
    {
        if (_statusBlock is null) return;
