...
namespace Netor.Cortana.UI.ViewModels.Common;

/// <summary>
/// 输入框 ViewModel 抽象接口。
///
/// <see cref="Controls.InputAreaView"/> 通过此接口与具体 VM 解耦。
///
/// 实现类：
/// <list type="bullet">
...
public interface IInputVm : INotifyPropertyChanged
{
    ...
    /// <summary>任务是否运行中（控制 Send/Stop 按钮显隐）。</summary>
    bool IsRunning { get; }
    ...
    /// <summary>
    /// 是否有活跃任务（控制走马灯/旋转动画持续性）。
    /// 专家模式（Chat）下等同于 IsRunning。
    /// </summary>
