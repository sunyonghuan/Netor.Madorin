    ...
    PluginLoader pluginLoader) : AIContextProvider
{
    ...
    private void RegisterTools()
    {
        ...
        _tools.Add(AIFunctionFactory.Create(
            name: "sys_load_plugin",
            description: "Loads a specified plugin directory for the first time. Parameter: dirName (plugin directory name under the global plugins directory). Used after installing a new plugin.",
            method: (string dirName) => LoadPluginAsync(dirName)));
    ...
    private static string BuildInstructions() =>
        """
        You can manage loaded plugins: view list, load, unload, and reload.
        Plugin install workflow: Copy the plugin directory under the global plugins directory → Call sys_load_plugin to load it.
    ...
    private async Task<string> LoadPluginAsync(string dirName)
    {
        if (string.IsNullOrWhiteSpace(dirName))
            return "✗ 插件目录名不能为空。";
        ...
        try
        {
            var pluginPath = Path.Combine(pluginLoader.PluginsDirectory, dirName);
            var loaded = await pluginLoader.LoadPluginByPathAsync(pluginPath);
            if (!loaded)
            {
                logger.LogWarning("加载插件未成功：{DirName}", dirName);
                return $"✗ 插件「{dirName}」加载未成功，请检查插件目录、plugin.json 或启动日志。";
            }

            logger.LogInformation("已加载插件：{DirName}", dirName);
            return $"✓ 已加载插件「{dirName}」。";
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "加载插件失败：{DirName}", dirName);
            return $"✗ 加载插件失败：{ex.Message}";
        }
    }
    ...
    private async Task<string> ReloadPluginAsync(string dirName)
    {
        ...
        try
        {
            var loaded = await pluginLoader.ReloadPluginAsync(dirName);
            if (!loaded)
            {
                logger.LogWarning("重载插件未成功：{DirName}", dirName);
                return $"✗ 插件「{dirName}」重载未成功，请检查插件目录、plugin.json 或启动日志。";
            }
