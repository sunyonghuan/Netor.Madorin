...
namespace Netor.Cortana.UI.Providers;
    ...
    IPublisher publisher) : AIContextProvider
{
    ...
    private AgentFileService AgentFileService => serviceProvider.GetRequiredService<AgentFileService>();
    private AgentManifestValidator AgentManifestValidator => serviceProvider.GetRequiredService<AgentManifestValidator>();
    private SystemSettingsService SettingsService => serviceProvider.GetRequiredService<SystemSettingsService>();
    ...
    private void RegisterTools()
    {
        ...
        _tools.Add(AIFunctionFactory.Create(
            name: "sys_add_agent",
            description: "Add a new agent (assistant/proxy/Agent). Parameters: name (stable ASCII name), instructions (system prompt).",
            method: (string name, string instructions) => AddAgent(name, instructions)));

        _tools.Add(AIFunctionFactory.Create(
            name: "sys_delete_agent",
            description: "Soft delete an agent by index. builtin/system agents cannot be deleted. Parameter: agentIndex (1-based index, call sys_list_agents first).",
            method: (int agentIndex) => DeleteAgent(agentIndex)));
    ...
    {
        var list = AgentService.GetSelectable();
        if (list.Count == 0)
            return "当前没有已启用的智能体。";
        ...
        {
            var a = list[i];
            var marker = AgentService.IsDefault(a.Id) ? " ★默认" : "";
    ...
    private string SetDefaultAgent(int index)
    {
        ...
        var target = list[index - 1];
        SettingsService.SetValue("Agent.DefaultName", target.Id);
    ...
    private string AddAgent(string name, string instructions)
    {
        try
        {
            var manifestName = NormalizeAgentName(name);
            if (AgentFileService.GetByName(manifestName) is not null)
                return $"✗ 智能体「{manifestName}」已存在。";

            var manifest = new AgentManifest
            {
                Name = manifestName,
                DisplayName = name.Trim(),
                Description = $"智能体 {name.Trim()}。",
            ...
            var validation = AgentManifestValidator.Validate(manifest, manifest.Name);
            if (!validation.IsValid)
                return $"✗ 新增智能体失败：{validation.Error}";

            AgentFileService.Save(manifest, instructions ?? string.Empty);
            publisher.Publish(Events.OnAgentChange, new DataChangeArgs(manifest.Name, ChangeType.Create));
            logger.LogInformation("已新增智能体：{Name}", name);

            return $"✓ 已新增智能体「{name}」[name={manifest.Name}]";
        }
        catch (Exception ex)
        {
    ...
    private string DeleteAgent(int agentIndex)
    {
        try
        {
            var list = AgentService.GetSelectable();
            if (agentIndex < 1 || agentIndex > list.Count)
                return $"✗ 无效的智能体序号 {agentIndex}，有效范围 1~{list.Count}。请先调用 sys_list_agents 查看列表。";

            var target = list[agentIndex - 1];
            AgentFileService.SoftDelete(target.Id);
            publisher.Publish(Events.OnAgentChange, new DataChangeArgs(target.Id, ChangeType.Delete));
            logger.LogInformation("已删除智能体：{Name}", target.Name);

            return $"✓ 已删除智能体「{target.Name}」。";
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "删除智能体失败");
            return $"✗ 删除智能体失败：{ex.Message}";
        }
    }
    ...
    private string GetAgentInstructions(int agentIndex)
    {
        try
        {
            ...
            {
                // 获取默认智能体的提示词
                var defaultAgent = AgentService.GetDefaultOrFirst();
                target = list.FirstOrDefault(a => a.Id == defaultAgent?.Id) ?? list[0];
            }
            else
            {
    ...
    private string UpdateAgentInstructions(int agentIndex, string instructions)
    {
        try
        {
            ...
            var target = list[agentIndex - 1];
            var record = AgentFileService.GetByName(target.Id);
            if (record is null)
                return $"✗ 智能体「{target.Name}」不存在。";

            AgentFileService.Save(record.Manifest, instructions ?? string.Empty);
    ...
    private string EnableMcpForAgent(int mcpIndex, int agentIndex)
    {
        try
        {
            ...
            var agents = AgentService.GetSelectable();
            if (agents.Count == 0)
                return "✗ 当前没有可用的智能体。";
            ...
            {
                var defaultAgent = AgentService.GetDefaultOrFirst();
                targetAgent = agents.FirstOrDefault(agent => agent.Id == defaultAgent?.Id) ?? agents[0];
            }
            else
            {
            ...
            var record = AgentFileService.GetByName(targetAgent.Id);
            if (record is null)
                return $"✗ 智能体「{targetAgent.Name}」不存在。";

            if (record.Manifest.BoundMcp.Contains(targetMcp.Id, StringComparer.OrdinalIgnoreCase))
                return $"✓ 智能体「{targetAgent.Name}」已启用 MCP 服务「{targetMcp.Name}」，无需重复设置。";

            record.Manifest.BoundMcp = [.. record.Manifest.BoundMcp, targetMcp.Id];
            AgentFileService.Save(record.Manifest, record.Prompt);
    ...
    private static string NormalizeAgentName(string name)
    {
        ...
        var chars = trimmed
            .Select(static c => char.IsAsciiLetterOrDigit(c) || c is '_' or '-' ? char.ToLowerInvariant(c) : '-')
            .ToArray();
        var normalized = new string(chars).Trim('-');

        return string.IsNullOrWhiteSpace(normalized)
            ? $"agent-{Guid.NewGuid():N}"[..14]
            : normalized.Length > 64 ? normalized[..64] : normalized;
    }
