...
[JsonSerializable(typeof(MeetingMessageCompletedArgs))]
...
// AllowMemoryIngest / CursorId 对齐见 Docs/已完成功能规划/长期记忆事件链路修复。
// ════════════════════════════════════════════════════════════════════════
...
internal sealed record WorkflowExportRecord
{
    ...
    [JsonPropertyName("allowMemoryIngest")] public bool AllowMemoryIngest { get; init; } = true;
}
...
/// <summary>
/// Meeting 历史回放批次负载（meeting.history.batch）。
/// </summary>
internal sealed record MeetingExportBatch
{
    [JsonPropertyName("batchId")] public string BatchId { get; init; } = string.Empty;
    [JsonPropertyName("hasMore")] public bool HasMore { get; init; }
    [JsonPropertyName("items")] public MeetingExportRecord[] Items { get; init; } = Array.Empty<MeetingExportRecord>();
}

/// <summary>
/// Meeting 会议级历史导出记录。
/// </summary>
internal sealed record MeetingExportRecord
{
    [JsonIgnore] public string CursorId { get; init; } = string.Empty;
    [JsonPropertyName("recordKind")] public string RecordKind { get; init; } = "final";
    [JsonPropertyName("meetingId")] public string MeetingId { get; init; } = string.Empty;
    [JsonPropertyName("messageId")] public string? MessageId { get; init; }
    [JsonPropertyName("sessionId")] public string SessionId { get; init; } = string.Empty;
    [JsonPropertyName("workspaceId")] public string WorkspaceId { get; init; } = string.Empty;
    [JsonPropertyName("topic")] public string Topic { get; init; } = string.Empty;
    [JsonPropertyName("status")] public int Status { get; init; }
    [JsonPropertyName("hostAgentId")] public string HostAgentId { get; init; } = string.Empty;
    [JsonPropertyName("participantsJson")] public string ParticipantsJson { get; init; } = "[]";
    [JsonPropertyName("contentMd")] public string? ContentMd { get; init; }
    [JsonPropertyName("messageRole")] public string? MessageRole { get; init; }
    [JsonPropertyName("finalSummaryMd")] public string? FinalSummaryMd { get; init; }
...
/// <summary>
/// Meeting 历史回放完成响应载荷（meeting.history.completed）。
/// </summary>
internal sealed record MeetingHistoryCompletedPayload
{
    [JsonPropertyName("total")] public int Total { get; init; }
}
