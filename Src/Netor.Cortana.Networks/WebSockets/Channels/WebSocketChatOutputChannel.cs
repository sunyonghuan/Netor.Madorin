    ...
    WebSocketRequestContext requestContext) : IAiOutputChannel
{
    ...
    public async Task OnDoneAsync(string turnId, string sessionId, CancellationToken cancellationToken = default)
    {
        ...
        logger.LogDebug("WebSocket 输出通道完成，Turn：{TurnId}，Session：{SessionId}", turnId, sessionId);
    }
    ...
    public async Task OnCancelledAsync(string turnId)
    {
        ...
        logger.LogDebug("WebSocket 输出通道已取消，Turn：{TurnId}", turnId);
    }
    ...
    public async Task OnErrorAsync(string turnId, string message, CancellationToken cancellationToken = default)
    {
        ...
        logger.LogWarning("WebSocket 输出通道收到错误，Turn：{TurnId}，Message：{Message}", turnId, message);
    }
}
