...
public sealed class WebSocketPluginBusServerService : KestrelWebSocketHost, IHostedService, IPluginBusBroadcaster, ILongMemorySupplyClient, IChatTransport, IDisposable
{
        ...
        PluginManifestRegistry manifestRegistry)
    {
        ...
        _workflowHistoryDispatcher = new PluginBusWorkflowHistoryDispatcher(db, agentService, logger, SendAsync);
        _meetingHistoryDispatcher = new PluginBusMeetingHistoryDispatcher(db, logger, SendAsync);
    ...
    private async Task HandleMessageAsync(string id, string json, CancellationToken ct)
    {
        try
        {
            ...
            if (string.Equals(type, "send", StringComparison.Ordinal)
                || string.Equals(op, "conversation.chat.message.send", StringComparison.Ordinal))
            {
            ...
            if (string.Equals(type, "stop", StringComparison.Ordinal)
                || string.Equals(op, "conversation.chat.generation.stop", StringComparison.Ordinal))
            {
            ...
            if (string.Equals(type, "subscribe", StringComparison.Ordinal))
            {
                ...
                // PluginBus 1.4.0：解析 subscribedOps 字段（缺省 = 订阅 topic 下全部 op）
                // 详见 docs/已完成功能规划/插件事件广播/README.md §2.2。
                var subscribedOps = root.TryGetProperty("subscribedOps", out var opsElement) && opsElement.ValueKind == JsonValueKind.Array
            ...
            if (string.Equals(type, "request", StringComparison.Ordinal)
                && string.Equals(op, CortanaWsEndpoints.MeetingHistoryReplayOperation, StringComparison.Ordinal))
            {
                var payload = root.TryGetProperty("payload", out var payloadElement) && payloadElement.ValueKind == JsonValueKind.Object
                    ? payloadElement
                    : root;
                var since = payload.TryGetProperty("sinceTimestamp", out var s) ? s.GetInt64() : 0L;
                var batch = payload.TryGetProperty("batchSize", out var b) ? Math.Clamp(b.GetInt32(), 100, 2000) : 500;
                var requestId = root.TryGetProperty("requestId", out var meetingRequestId) ? meetingRequestId.GetString() : null;
                await _meetingHistoryDispatcher.ReplayAsync(id, requestId, since, batch, ct); return;
            }
