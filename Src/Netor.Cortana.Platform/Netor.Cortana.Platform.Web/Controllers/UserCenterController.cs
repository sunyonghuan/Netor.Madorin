...
public sealed class UserCenterController(PlatformDbContext dbContext) : WebControllerBase
{
    ...
    private async Task<IReadOnlyList<UserSubscriptionItem>> GetSubscriptionItems(string accountId, CancellationToken cancellationToken, bool onlyExpiringSoon = false)
    {
        ...
        var items = rows.Select(x =>
        {
            ...
            var currentVersion = x.Asset?.Versions.FirstOrDefault(version => version.ID == x.Asset.CurrentVersionId);
            return new UserSubscriptionItem(
                ...
                x.ExpiresAtUtc <= now,
                x.Asset?.Type == AssetType.Solution ? ParseSolutionSummary(currentVersion?.ManifestJson) : null);
        }).ToList();
    ...
    {
        var accountId = CurrentAccountId!;
        var rows = await dbContext.DownloadRecords
            .AsNoTracking()
            .Include(x => x.Asset)
        ...
        var items = rows
            .Select(x => new UserDownloadItem(
                ...
                x.IpAddress,
                x.TimeStamp,
                x.Asset != null && x.Asset.Type == AssetType.Solution
                    ? ParseSolutionSummary(x.AssetVersion == null ? null : x.AssetVersion.ManifestJson)
                    : null))
            .ToList();
