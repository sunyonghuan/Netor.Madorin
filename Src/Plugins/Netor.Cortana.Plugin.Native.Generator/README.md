...
| 导出函数 | 说明 |
|----------|------|
| `madorin_plugin_get_info` | 返回插件和工具的完整元数据 JSON |
| `madorin_plugin_init` | 初始化 DI 容器、注册服务、启动 `IHostedService` |
| `madorin_plugin_invoke` | 按工具名路由调用，自动参数解析和错误处理 |
| `madorin_plugin_free` | 释放 `Marshal.StringToCoTaskMemUTF8` 分配的内存 |
| `madorin_plugin_destroy` | 停止 `IHostedService`，Dispose `ServiceProvider` |
...
> 通常不需要单独安装此包，安装 `Netor.Madorin.Plugin.Native` 时会自动引入。
...
└── build/
    └── Netor.Madorin.Plugin.Native.Generator.targets  # MSBuild 目标
```
