# Netor.Madorin.Plugin.Process.Generator

Madorin Process 通道插件框架的 Roslyn Source Generator，负责根据标记代码生成入口、清单和调试辅助代码。
...
> 正常消费场景只需引用 `Netor.Madorin.Plugin.Process`。它会自动携带 Generator analyzer 与 `buildTransitive` 目标；通常不需要单独引用 Generator 包。
...
- `build/Netor.Madorin.Plugin.Process.Generator.targets` 负责把生成内容参与构建输出
...
| 诊断 ID | 说明 |
|------|------|
| `CPPG003` | 工具方法使用了不支持的参数类型 |
| `CPPG004` | 工具名冲突 |
| `CPPG005` | 类不是 public，或 `[Tool]` 方法所属类缺少 `[Tool]` 标记 |
| `CPPG006` | 工具方法不是 public |
| `CPPG007` | 方法名已是 snake_case（警告） |
| `CPPG008` | 转换后的工具名非法 |
| `CPPG009` | `[Plugin]` 缺少必填属性 |
| `CPPG010` | 存在多个 `[Plugin]` 入口类 |
| `CPPG011` | `[Plugin]` 入口类不是 partial class |
| `CPPG019` | `[Plugin].Id` 格式不合法 |
| `CPPG020` | 返回自定义类型但项目缺少 `PluginJsonContext` |
| `CPPG021` | `PluginSettingType.Json` 的 `DefaultValue` 不是合法 JSON |
...
- 依赖 Roslyn 编译器 API
- 主要面向 `Netor.Madorin.Plugin.Process` 消费项目使用
