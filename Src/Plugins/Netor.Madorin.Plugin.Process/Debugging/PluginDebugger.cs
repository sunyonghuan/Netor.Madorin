...
using Netor.Madorin.Plugin.Process.Protocol;
...
public abstract class PluginDebugger : IAsyncDisposable
{
        ...
        int wsPort = 0)
    {
        var config = new InitConfig
        {
            ...
            {
                ["pluginBusEndpoint"] = $"ws://localhost:{wsPort}/internal",
                ["pluginBusPort"] = wsPort.ToString(System.Globalization.CultureInfo.InvariantCulture),
    ...
    /// <summary>
    /// 发送一条原始宿主请求并返回原始响应 JSON。
    /// 用于覆盖强类型 Debugger 难以表达的协议级回归场景。
    /// </summary>
    public async Task<string> InvokeRawAsync(HostRequest request)
    {
        ArgumentNullException.ThrowIfNull(request);

        var response = await SendRequestAsync(request).ConfigureAwait(false);
        return JsonSerializer.Serialize(response, ProcessProtocolJsonContext.Default.HostResponse);
    }
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        {
            Console.Error.WriteLine($"PluginDebugger dispose observed plugin task failure: {ex}");
        }
