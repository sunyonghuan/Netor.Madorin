...
Madorin 原生插件开发框架的**运行时库**，提供 Attribute 标记和运行时类型，配合 `Netor.Cortana.Plugin.Native.Generator` 源码生成器使用。
...
- .NET 10+
- 引用 `Netor.Cortana.Plugin.Native` 即可自动获得 Generator 与 `buildTransitive` 行为；通常不需要额外单独引用 Generator 包
