...
public static class CortanaWsEndpoints
{
    ...
    /// 阶段会议模式 WebSocket 分发：从 1.2.0 升到 1.3.0，新增 meeting topic 与对应 operations。
    /// 插件事件广播：从 1.3.0 升到 1.4.0，type=event 帧扩展为支持 plugin→plugin 转发，
    /// PluginBusEventMessage 新增 hopCount / sourcePluginId 字段，subscribe 帧新增 subscribedOps 字段；
    /// op 优先级有意变更为 op &gt; eventType &gt; type；详见 docs/已完成功能规划/插件事件广播/README.md §2.1。
    /// 详见 docs/未来版本策划/多智能体编排模式策划/07-事件分流与插件兼容设计.md §3.1
    /// 与 04-实施阶段.md §5B.4 / Phase 4 实施计划 §5.2。
    /// 兼容性：旧 1.0.0 / 1.1.0 / 1.2.0 / 1.3.0 插件连接到 1.4.0 宿主仍然可用（缺少 capabilities / subscribedOps 时仅记 warning 不断连，
    /// 决策 5B-D：避免单点故障导致全部插件断连）。
    /// </summary>
    public const string PluginBusVersion = "1.4.0";
    ...
    public const string CapabilityMeetingV1 = "meeting.v1";
