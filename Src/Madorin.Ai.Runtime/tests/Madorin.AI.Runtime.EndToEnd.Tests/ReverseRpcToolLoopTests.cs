...
public sealed class ReverseRpcToolLoopTests
{
    ...
    private const string ProcessToolId = "host.process.exec";
    ...
    [DoNotParallelize]
    [Timeout(30_000, CooperativeCancellation = true)]
    public async Task WorkParentRunCancellation_CancelsChildProcessToolAndKeepsOtherRunAlive()
    {
        var workspace = CreateTemporaryDirectory();
        var instanceId = Guid.NewGuid().ToString("N");
        var secret = Convert.ToBase64String(RandomNumberGenerator.GetBytes(32));
        var workProvider = new CancellableWorkProvider();
        var independentProvider = new IndependentBlockingProvider();
        using var shutdown = CancellationTokenSource.CreateLinkedTokenSource(
            TestContext.CancellationToken);
        try
        {
            await using var server = await RuntimeServer.StartAsync(
                ...
                {
                    InstanceId = instanceId,
                    PipePrefix = $"madorin.work-cancel.{Guid.NewGuid():N}",
                    HandshakeSecret = secret,
                    MaxConcurrentRuns = 2,
                    MaxConcurrentRunsPerProvider = 1,
                    MemoryUserHome = Path.Combine(workspace, "home"),
                    ProviderResolver = selection =>
                        selection.DefaultSelection.ProviderId == workProvider.ProviderId
                            ? workProvider
                            : independentProvider
                },
                TestContext.CancellationToken);
            ...
            var host = new CancellableProcessToolHost(workspace);
            client.ControlPeer.SetRequestHandler(host.HandleAsync);
            await PublishCatalogAsync(client, timeout.Token);

            var workRunId = await client.StartNewSessionRunAsync(
                CreateCancellableWorkRequest(),
                timeout.Token);
            var workTerminalTask = ReadTerminalRunEventAsync(client, workRunId, timeout.Token);
            var processStartedOrTerminal = await Task.WhenAny(
                host.ProcessStarted,
                workTerminalTask);
            if (ReferenceEquals(processStartedOrTerminal, workTerminalTask))
            {
                var earlyTerminal = await workTerminalTask;
                var detail = earlyTerminal.MessageType == MessageTypes.RunFailed
                    ? JsonSerializer.Deserialize(
                        earlyTerminal.Payload,
                        RuntimeJsonContext.Default.RunFailedEvent)?.Error.Message
                    : earlyTerminal.Payload.GetRawText();
                Assert.Fail(
                    $"The Work run terminated before the process tool started: {earlyTerminal.MessageType}: {detail}");
            }

            var independentRunId = await client.StartNewSessionRunAsync(
                CreateIndependentRequest(),
                timeout.Token);
            await independentProvider.Started.WaitAsync(timeout.Token);

            await client.CancelRunAsync(workRunId, timeout.Token);
            var cancelRequest = await host.ProcessCancelled.WaitAsync(timeout.Token);
            var workTerminal = await workTerminalTask;

            var workFailure = workTerminal.MessageType == MessageTypes.RunFailed
                ? JsonSerializer.Deserialize(
                    workTerminal.Payload,
                    RuntimeJsonContext.Default.RunFailedEvent)?.Error.Message
                : null;
            var persistedToolStatus = await ReadToolIntentStatusAsync(
                workspace,
                "work-process-call",
                timeout.Token);
            Assert.AreEqual(
                MessageTypes.RunCancelled,
                workTerminal.MessageType,
                $"{workFailure}; persisted tool status: {persistedToolStatus}");
            Assert.AreEqual("work-process-call", cancelRequest.CallId);
            Assert.AreEqual("runtime-cancelled", cancelRequest.Reason);
            Assert.AreEqual(1, host.ProcessExecutionCount);
            var workRun = await client.QueryRunAsync(workRunId, timeout.Token);
            Assert.AreEqual(RunStatus.Cancelled, workRun.Status);
            var resumed = await client.ResumeSessionAsync(workRun.SessionId, timeout.Token);
            Assert.IsNotNull(resumed.WorkState);
            Assert.AreEqual(WorkSessionStatus.Interrupted, resumed.WorkState.Status);
            var interruptedStep = Assert.ContainsSingle(resumed.WorkState.Steps);
            Assert.AreEqual(WorkStepLifecycleStatus.Interrupted, interruptedStep.Status);
            Assert.IsNotNull(interruptedStep.InvocationId);
            await AssertLatestAttemptInterruptedAsync(
                workspace,
                interruptedStep.InvocationId,
                timeout.Token);

            var unexpectedCancellation = await Task.WhenAny(
                independentProvider.CancellationObserved,
                Task.Delay(TimeSpan.FromMilliseconds(100), timeout.Token));
            Assert.AreNotSame(independentProvider.CancellationObserved, unexpectedCancellation);
            var independentRun = await client.QueryRunAsync(independentRunId, timeout.Token);
            Assert.AreEqual(RunStatus.Running, independentRun.Status);

            await client.CancelRunAsync(independentRunId, timeout.Token);
            await independentProvider.CancellationObserved.WaitAsync(timeout.Token);
        ...
        CancellationToken ct)
    {
        var request = new ToolCatalogReplaceRequest(
            ...
            [
                ...
                CreateCatalogItem(ProcessToolId, ToolExecutionTarget.Host, requiresApproval: false)
            ]);
    ...
    private static NewSessionRunRequest CreateCancellableWorkRequest()
    {
        var manager = new AgentRef(
            "manager",
            "v1",
            "Create a one-step process plan.",
            "work-provider",
            "work-model");
        var worker = new AgentRef(
            "worker",
            "v1",
            "Run the assigned process tool.",
            "work-provider",
            "work-model");
        return new NewSessionRunRequest(
            ...
            new NextTurnSelection(
                1,
                RuntimeMode.Work,
                new DefaultSelection("work-provider", "work-model"),
                new WorkModeOptions(manager, [worker]),
                ToolCatalogVersion: "host-v1"),
            [new TextContentBlock("Run the process until cancelled.")]);
    }

    private static NewSessionRunRequest CreateIndependentRequest() =>
        new(
            "independent-session",
            "independent-run",
            RuntimeMode.Expert,
            new NextTurnSelection(
                1,
                RuntimeMode.Expert,
                new DefaultSelection("independent-provider", "independent-model"),
                new ExpertModeOptions(
                    new AgentRef(
                        "independent-agent",
                        "v1",
    ...
    private static async Task AssertLatestAttemptInterruptedAsync(
        string workspace,
        string invocationId,
        CancellationToken ct)
    {
        ...
        Assert.AreEqual("interrupted", await command.ExecuteScalarAsync(ct));
    }

    private static async Task<string?> ReadToolIntentStatusAsync(
        string workspace,
        string callId,
        CancellationToken ct)
    {
        ...
        await connection.OpenAsync(ct);
        await using var command = connection.CreateCommand();
        command.CommandText = "SELECT status FROM tool_intents WHERE call_id = $callId;";
    ...
    private sealed class CancellableWorkProvider : IRuntimeProviderAdapter
    {
        private readonly ConcurrentDictionary<string, int> _agentCalls = new(StringComparer.Ordinal);

        public string ProviderId => "work-provider";
        ...
        public IReadOnlyList<ProviderModel> Models { get; } =
            [new("work-model", "Work Model", ContextWindow: 16_384)];

        public async IAsyncEnumerable<RuntimeProviderEvent> CompleteStreamingAsync(
            RuntimeProviderRequest request,
            [EnumeratorCancellation] CancellationToken ct = default)
        {
            var call = _agentCalls.AddOrUpdate(request.AgentId, 1, static (_, count) => count + 1);
            await Task.Yield();
            ct.ThrowIfCancellationRequested();
            if (string.Equals(request.AgentId, "manager", StringComparison.Ordinal))
            {
                ...
                yield return new TextDeltaProviderEvent(request.InvocationId, "Create the process step.");
                yield return new InvocationCompletedProviderEvent(request.InvocationId, "stop");
                yield break;
            }
            ...
            if (call > 1)
            {
                ...
                yield return new InvocationFailedProviderEvent(
                    ...
                    new RuntimeError(
                        ...
                        "Test",
                        $"The process tool did not start: {detail}",
                        IsRetryable: false,
                        ProviderDetails: null,
                        DiagnosticId: "work-process-tool-did-not-start"));
                yield break;
            }
    ...
    private sealed class IndependentBlockingProvider : IRuntimeProviderAdapter
    {
        ...
        public string ProviderId => "independent-provider";

        public ProviderCapabilities Capabilities { get; } = new(Streaming: true);

        public IReadOnlyList<ProviderModel> Models { get; } =
            [new("independent-model", "Independent Model", ContextWindow: 8192)];
    ...
    private sealed class CancellableProcessToolHost(string workspace)
    {
        private readonly TaskCompletionSource _processStarted = new(
            TaskCreationOptions.RunContinuationsAsynchronously);
        private readonly TaskCompletionSource<ToolCallCancelRequest> _processCancelled = new(
            TaskCreationOptions.RunContinuationsAsynchronously);
        private readonly DateTimeOffset _grantExpiresAt = DateTimeOffset.UtcNow.AddMinutes(10);
        ...
        public Task<ToolCallCancelRequest> ProcessCancelled => _processCancelled.Task;
            ...
            CancellationToken ct)
        {
            ...
            {
                case MessageTypes.ToolPermissionRequest:
                    return HandlePermission(request);
                case MessageTypes.ToolCallRequest:
                    return await HandleToolCallAsync(request, ct);
                case MessageTypes.ToolResultQuery:
                    return HandleResultQuery(request);
                case MessageTypes.ToolCallCancel:
        ...
        private JsonRpcResponse HandlePermission(JsonRpcRequest request)
        {
            ...
            var isManager = string.Equals(permission.AgentId, "manager", StringComparison.Ordinal);
            var grantId = isManager ? "grant-manager" : "grant-worker";
            var grant = new ToolGrant(
                ...
                new NetworkPolicy(),
                _grantExpiresAt,
                AllowDelegation: isManager,
                DelegatedAgentIds: isManager ? ["worker"] : [],
                AllowedToolIds: [ProcessToolId],
                AllowedCallIds: isManager ? null : [permission.CallId],
                MaximumRisk: ToolRiskLevel.Low,
                AgentId: permission.AgentId,
                ParentGrantId: isManager ? null : "grant-manager",
                RootGrantId: "grant-manager",
                DelegationChain: isManager ? null : ["grant-manager"]);
            ...
            CancellationToken ct)
        {
            ...
            if (!string.Equals(call.CallId, "work-process-call", StringComparison.Ordinal))
            {
                var primed = new ToolCallResponse(
                    call.CorrelationId,
                    call.CallId,
                    ToolCallStatus.Succeeded,
                    ParseElement("""{"primed":true}"""));
                return CreateResponse(request, primed, RuntimeJsonContext.Default.ToolCallResponse);
            }

            Interlocked.Increment(ref _processExecutionCount);
            _processStarted.TrySetResult();
            _ = await _processCancelled.Task.WaitAsync(ct);
            var cancelled = new ToolCallResponse(
                call.CorrelationId,
                call.CallId,
                ToolCallStatus.Cancelled,
                ErrorCode: "Cancelled",
                ErrorMessage: "The process tool was cancelled by the Runtime.");
            return CreateResponse(request, cancelled, RuntimeJsonContext.Default.ToolCallResponse);
        }
        ...
        private JsonRpcResponse HandleResultQuery(JsonRpcRequest request)
        {
            ...
            var isProcessCall = string.Equals(
                query.CallId,
                "work-process-call",
                StringComparison.Ordinal);
            var status = isProcessCall && _processCancelled.Task.IsCompleted
                ? ToolCallStatus.Cancelled
                : isProcessCall && _processStarted.Task.IsCompleted
                    ? ToolCallStatus.Running
                    : ToolCallStatus.Unknown;
            var response = new ToolResultQueryResponse(
                query.CorrelationId,
                query.CallId,
                status,
                ErrorCode: status == ToolCallStatus.Cancelled ? "Cancelled" : null,
                ErrorMessage: status == ToolCallStatus.Cancelled
                    ? "The process tool was cancelled by the Runtime."
                    : null);
            return CreateResponse(
                request,
                response,
        ...
        {
            var cancel = Deserialize(request, RuntimeJsonContext.Default.ToolCallCancelRequest);
            _processCancelled.TrySetResult(cancel);
            return CreateResponse(request, true, RuntimeJsonContext.Default.Boolean);
        }
