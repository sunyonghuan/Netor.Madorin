...
public static class RunStateMachine
{
    ...
    {
        if (IsTerminal(current) || current is RunStatus.Interrupted)
        {
            return false;
        }
        ...
        return (current, next) switch
        {
            ...
            (RunStatus.WaitingForTool, RunStatus.Running or RunStatus.Failed) => true,
            (RunStatus.WaitingForApproval, RunStatus.Running or RunStatus.Failed) => true,
