...
/// <summary>
/// Converts between runtime canonical <see cref="ContentBlock"/> format
/// and the MEAI/Anthropic message format.
/// </summary>
            ...
            foreach (var block in message.Content)
            {
                content.Add(block switch
                {
                    ...
                    _ => throw new InvalidOperationException(
                        $"ContentBlock type '{block.GetType().Name}' is not supported by the Anthropic adapter.")
                });
            }
