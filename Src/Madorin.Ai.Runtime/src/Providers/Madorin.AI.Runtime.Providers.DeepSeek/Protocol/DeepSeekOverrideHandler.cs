...
/// <summary>
/// 拦截 DeepSeek 请求，在最终 JSON 中为所有 assistant 消息补写
/// <c>reasoning_content</c> 字段（DeepSeek-R1 工具调用多轮要求）。
/// 从老项目 <c>Netor.Cortana.AI/Drivers/Providers/Deepseek/DeepseekOverrideHandler.cs</c> 迁移。
/// </summary>
internal sealed partial class DeepSeekOverrideHandler(ILogger<DeepSeekOverrideHandler> logger) : DelegatingHandler
{
    ...
    /// <summary>
    /// 在聊天请求体中为所有 assistant 消息补写 reasoning_content：
    /// 最后一条带 tool_calls 的 assistant 写入真实 reasoning，其余写空字符串。
    /// </summary>
    private static string RewriteRequestBody(string originalBody)
    {
        ...
        var reasoning = DeepSeekReasoningContext.CurrentReasoning ?? string.Empty;
            ...
            foreach (var property in document.RootElement.EnumerateObject())
            {
                if (property.NameEquals("messages"))
                {
                    ...
                    foreach (var message in messages.EnumerateArray())
                    {
                        ...
                        {
                            WriteAssistantMessageWithReasoning(
                                writer,
                                message,
                                index == lastAssistantIndex ? reasoning : string.Empty);
                            injected = true;
                        }
                        else
                        {
    ...
    private static void WriteAssistantMessageWithReasoning(
        Utf8JsonWriter writer, JsonElement message, string reasoning)
    {
        writer.WriteStartObject();
    ...
    [LoggerMessage(Level = LogLevel.Debug, Message = "DeepSeek 请求已取消。")]
