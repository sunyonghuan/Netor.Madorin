...
/// <summary>
/// PowerShell 会话管理器 - 管理所有活跃的本机托管交互式 PowerShell 会话
/// </summary>
public sealed class SessionRegistry : IAsyncDisposable
{
    ...
    /// 创建并注册一个新会话
    /// </summary>
    public ExecutionSession CreateSession(bool background = true)
    {
        var session = new ExecutionSession(_logger, background);
        ...
        _logger.LogInformation("PowerShell 会话已创建: {SessionId}", session.Id);
        return session;
    }
    ...
    /// 列出所有活跃会话
    /// </summary>
    public List<(string Id, string Type, DateTime CreatedAt, ExecutionSessionState State, string StatusMessage)> GetActiveSessions()
    {
        lock (_lock)
        {
            return _sessions
                .Where(x => x.Value.IsActive)
                .Select(x => (x.Key, x.Value.Type, x.Value.CreatedAt, x.Value.State, x.Value.StatusMessage))
                .ToList();
        }
    }
